...
export default function ImportPatientsModal({ onClose, onImport, existingPatients }: ImportPatientsModalProps) {
  ...
  const templateColumns = [
    { key: 'name', label: 'Nome Completo', required: true, example: 'Maria Aparecida da Silva' },
    { key: 'cpf', label: 'CPF', required: true, example: '123.456.789-00' },
    { key: 'birthDate', label: 'Data de Nascimento', required: true, example: '1978-05-13' },
    { key: 'gender', label: 'Gênero', required: false, example: 'Feminino' },
    { key: 'phone', label: 'Telefone', required: false, example: '(11) 98888-7777' },
    { key: 'email', label: 'Email', required: false, example: 'maria@example.com' },
    { key: 'recordNumber', label: 'Prontuário', required: false, example: 'PR-1001' },
  ];
  ...
  const processFile = async () => {
    ...
    reader.onload = (e) => {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        // Duplicate Check (CPF)
        const cpfVal = patient.cpf.replace(/\D/g, '');
        
        // 1. Check within file
        if (processedCpfs.has(cpfVal)) {
          newErrors.push(`Linha ${i + 1}: Paciente duplicado no arquivo (CPF ${patient.cpf} já processado).`);
          continue;
        }

        // 2. Check against existing patients
        const isExisting = existingPatients.some(p => 
          p.cpf.replace(/\D/g, '') === cpfVal
        );

        if (isExisting) {
          newErrors.push(`Linha ${i + 1}: Paciente já cadastrado no sistema (CPF ${patient.cpf}).`);
          continue;
        }

        processedCpfs.add(cpfVal);
        importedPatients.push(patient as Patient);
      }
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-md p-2 sm:p-4 animate-in fade-in duration-300">
      <div className="bg-white dark:bg-slate-900 rounded-2xl sm:rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col border border-white dark:border-slate-800 animate-in zoom-in-95 duration-200">
        ...
        <div className="px-4 py-4 sm:px-6 sm:py-5 overflow-y-auto max-h-[70vh] no-scrollbar">
          {/* Instructions Box */}
          <div className="mb-4 p-4 bg-indigo-50/50 dark:bg-indigo-900/20 rounded-xl border border-indigo-100 dark:border-indigo-900/30 relative overflow-hidden group">
