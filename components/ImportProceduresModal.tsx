...
interface ImportProceduresModalProps {
  onClose: () => void;
  onImport: (procedures: Procedure[]) => void;
  existingProcedures: Procedure[];
}

export default function ImportProceduresModal({ onClose, onImport, existingProcedures }: ImportProceduresModalProps) {
  ...
  const templateColumns = [
    { key: 'name', label: 'Nome do Procedimento', required: true, example: 'Ultrassom de Abdome Total' },
    { key: 'modality', label: 'Modalidade', required: true, example: 'US' },
    { key: 'category', label: 'Categoria', required: false, example: 'Exame' },
    { key: 'price', label: 'Valor (R$)', required: true, example: '380,00' },
    { key: 'preparation', label: 'Preparo', required: false, example: 'Jejum de 8 horas.' },
  ];
  ...
  const downloadTemplate = () => {
    ...
    link.setAttribute('download', 'modelo_importacao_procedimentos.csv');
  ...
  const processFile = async () => {
    ...
    reader.onload = (e) => {
      ...
      const importedProcedures: Procedure[] = [];
      const newErrors: string[] = [];
      const processedKeys = new Set<string>();

      for (let i = 1; i < lines.length; i++) {
        const values = lines[i].split(';').map(v => v.trim());
        const proc: any = { id: Math.random().toString(36).substr(2, 9), integraRis: false };
        let rowErrors = false;
        ...
        templateColumns.forEach((col, index) => {
          ...
          if (col.key === 'price' && val) {
            // Convert "380,00" or "380.00" to "380.00"
            val = val.replace(',', '.');
            if (isNaN(parseFloat(val))) {
              newErrors.push(`Linha ${i + 1}: Valor "${val}" é inválido.`);
              rowErrors = true;
            }
          }
        ...
        // Duplicate Check (Name + Modality)
        const duplicateKey = `${proc.name.toLowerCase()}-${proc.modality.toLowerCase()}`;
        
        // 1. Check within file
        if (processedKeys.has(duplicateKey)) {
          newErrors.push(`Linha ${i + 1}: Procedimento duplicado no arquivo ("${proc.name}" - ${proc.modality}).`);
          continue;
        }

        // 2. Check against existing procedures
        const isExisting = existingProcedures.some(p => 
          p.name.toLowerCase() === proc.name.toLowerCase() && 
          p.modality.toLowerCase() === proc.modality.toLowerCase()
        );

        if (isExisting) {
          newErrors.push(`Linha ${i + 1}: Procedimento já cadastrado no sistema ("${proc.name}" - ${proc.modality}).`);
          continue;
        }

        processedKeys.add(duplicateKey);
        importedProcedures.push(proc as Procedure);
      }
      ...
      if (newErrors.length > 0) {
        ...
        onImport(importedProcedures);
        toast.success(`${importedProcedures.length} procedimentos importados com sucesso!`);
        setIsProcessing(false);
        onClose();
      }
    };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-md p-2 sm:p-4 animate-in fade-in duration-300">
      <div className="bg-white dark:bg-slate-900 rounded-2xl sm:rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col border border-white dark:border-slate-800 animate-in zoom-in-95 duration-300">
        <div className="px-5 py-3 sm:px-6 sm:py-4 border-b border-slate-50 dark:border-slate-800 flex justify-between items-center bg-slate-50/30 dark:bg-slate-900/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-900 dark:text-white">Importar Procedimentos</h2>
