  ...
  return (
    <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-3 md:px-8 sticky top-0 z-30">
      ...
      <div className="flex items-center gap-2 lg:gap-4">
        <div id="header-search" className="hidden md:flex items-center bg-slate-100 rounded-lg px-3 py-1.5 transition-all">
        ...
        <div className="relative">
          <button 
            ...
            className={cn(
              ...
              showHelpTray ? "bg-indigo-50 text-indigo-600" : "hover:bg-slate-100 text-slate-600"
            )}
          ...
          <AnimatePresence>
            {showHelpTray && (
              <motion.div 
                ...
                className="absolute right-0 mt-3 w-72 bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden z-50 p-2"
              >
                <button 
                  onClick={() => {
                    onStartTour();
                ...
                >
                  ...
                  <div>
                    <p className="text-sm font-bold text-slate-900">Tour do Sistema</p>
                    <p className="text-[10px] text-slate-500">Conheça as ferramentas</p>
                  </div>
                </button>

                <button className="w-full flex items-center gap-3 p-3 hover:bg-slate-50 rounded-xl transition-all group text-left">
                  ...
                  <div>
                    <p className="text-sm font-bold text-slate-900">Suporte Técnico</p>
                    <p className="text-[10px] text-slate-500">Abra um chamado</p>
                  </div>
                </button>
              </motion.div>
            )}
            ...
            {showNotifications && (
              ...
              >
                <div className="p-4 border-b border-slate-50 bg-slate-50/50 flex items-center justify-between">
                  <p className="text-xs font-bold text-slate-900 uppercase tracking-wider">Últimas Ações</p>
                  <span className="text-[9px] bg-indigo-100 text-indigo-600 px-1.5 py-0.5 rounded-full font-bold">RECENTE</span>
                </div>
                <div className="max-h-96 overflow-y-auto p-2 space-y-1">
                  {notifications.map((n) => (
                    <div key={n.id} className="p-3 hover:bg-slate-50 rounded-xl transition-all flex gap-3 group cursor-default">
                      <div className={cn("w-8 h-8 rounded-lg bg-slate-50 flex items-center justify-center shrink-0", n.color)}>
                        <n.icon size={16} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start">
                          <p className="text-[12px] font-bold text-slate-900 truncate">{n.title}</p>
                ...
                <div className="p-3 border-t border-slate-50 text-center">
                  <button className="text-[10px] font-bold text-indigo-600 hover:underline px-4 py-2">VER TODO O HISTÓRICO</button>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
