...
interface ImportDoctorsModalProps {
  onClose: () => void;
  onImport: (doctors: Doctor[]) => void;
  existingDoctors: Doctor[];
}

export default function ImportDoctorsModal({ onClose, onImport, existingDoctors }: ImportDoctorsModalProps) {
  ...
  const templateColumns = [
    { key: 'name', label: 'Nome Completo', required: true, example: 'Dr. João Silva' },
    { key: 'crm', label: 'CRM', required: true, example: '12345-SP' },
    { key: 'specialty', label: 'Especialidade', required: true, example: 'Cardiologia' },
    { key: 'cpf', label: 'CPF', required: false, example: '123.456.789-00' },
    { key: 'type', label: 'Atuação', required: true, example: 'executante, solicitante ou ambos' },
    { key: 'phone', label: 'Telefone', required: false, example: '(11) 99999-9999' },
    { key: 'email', label: 'Email', required: false, example: 'medico@exemplo.com' },
  ];
  ...
  const downloadTemplate = () => {
    ...
    link.setAttribute('download', 'modelo_importacao_medicos.csv');
  ...
  const processFile = async () => {
    ...
    reader.onload = (e) => {
      ...
      const processedKeys = new Set<string>(); // To check within file
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        templateColumns.forEach((col, index) => {
          ...
          if (col.key === 'type' && val) {
            ...
            if (!['executante', 'solicitante', 'ambos'].includes(type)) {
              newErrors.push(`Linha ${i + 1}: O tipo "${val}" é inválido. Use: executante, solicitante ou ambos.`);
              rowErrors = true;
            }
          }
        ...
        // Duplicate Check (CRM and CPF)
        const crmVal = doctor.crm.replace(/\D/g, '').toLowerCase();
        const cpfVal = doctor.cpf.replace(/\D/g, '');
        const duplicateKey = `${crmVal}-${cpfVal}`;

        // 1. Check within file
        if (processedKeys.has(duplicateKey)) {
          newErrors.push(`Linha ${i + 1}: Médico duplicado no arquivo (CRM ${doctor.crm} e CPF ${doctor.cpf} já processados).`);
          continue;
        }

        // 2. Check against existing doctors
        const isExisting = existingDoctors.some(d => 
          d.crm.replace(/\D/g, '').toLowerCase() === crmVal && 
          d.cpf.replace(/\D/g, '') === cpfVal
        );

        if (isExisting) {
          newErrors.push(`Linha ${i + 1}: Médico já cadastrado no sistema (CRM ${doctor.crm} e CPF ${doctor.cpf}).`);
          continue;
        }

        processedKeys.add(duplicateKey);
        importedDoctors.push(doctor as Doctor);
      }
